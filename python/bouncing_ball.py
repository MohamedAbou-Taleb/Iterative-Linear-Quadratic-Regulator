...
def R_x(x_vec, y_vec, qk, uk):
    ...
    # Note: x(1:2) is dq, x(3:4) is du. uk is u_k. 
    # So: dq - dt * (uk + du) = 0
    res_pos = dq - dt * (du + uk)
...
def J_x_fun(x_vec, y_vec):
    """Jacobian of Dynamics Residual with respect to x_vec."""
    # J = [ I_2   -dt*I_2 ]
    #     [ 0_2    M      ]
    
    top = jnp.hstack([jnp.eye(2), -dt * jnp.eye(2)])
    bottom = jnp.hstack([jnp.zeros((2, 2)), m * jnp.eye(2)])
...
def solve_contact_system_jvp(qk, uk, r_T, r_N, primals, tangents):
    ...
    # We use jvp to compute (dG/d_params * d_params) efficiently
    # tangents contains (dx_guess, dy_guess). We strictly need gradients wrt qk, uk implicit in the closure.
    # However, custom_jvp arguments align with the decorated function arguments.
    # The decorated signature is (x_guess, y_guess, qk, uk, r_T, r_N).
    # Tangents passed to this function correspond to these arguments.
    # We only care about dqk (tangents[2]) and duk (tangents[3]).
...
def step_fn(carrier, t_curr):
    ...
    # M is diagonal [m, m]. InvM is [1/m, 1/m].
    # W = [W_T, W_N] = [[1, 0], [0, 1]] (Identity)
    # G_diag = diag(W' * M^-1 * W) = [1/m, 1/m]
    
    G_diag_T = 1.0/m
    G_diag_N = 1.0/m
    
    r_T = 1.0 / (dt * G_diag_T)
    r_N = 1.0 / (dt * G_diag_N) # Correction: MATLAB uses r_N = r_vec(2), not scaled by dt?
    # MATLAB: r_vec = 1.0 ./ (dt * G_diag);
    # r_T = r_vec(1)*dt; -> (1/(dt*G))*dt = 1/G
    # r_N = r_vec(2);    -> 1/(dt*G)
    ...
    # x_warm, y_warm come from previous step (warm starting)
...
def plot_results(t, q, u, PN, PT, iters):
    ...
    # 2. Velocities
    # Adjust t to match size (history length vs full length)
    # scan returns N steps. tspan is N steps. q_full is N+1. 
    # We'll plot u_full which is N+1 against t augmented
