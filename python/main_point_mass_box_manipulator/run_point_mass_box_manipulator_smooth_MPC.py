...
from class_files.systems.point_mass_box_manipulator_smooth_sys import MyPointMassBoxManipulatorSmooth # <-- Import MyDoublePendulum
...
def main():
    ...
    R = jnp.diag(jnp.array([1.0, 1.0, 1.0, 1.0]))*1e-2
    Q_box = jnp.diag(jnp.array([1.0, 100.0, 0.0, 0.0]))*1
    Q_f = jnp.diag(jnp.array([1.0, 1.0, 1.0, 1.0]))*1
    RN1 = 0; RN2 = 0; RN1_f = 0.0; RN2_f = 0.0
    m_box = 0.5
    m_ball = 1
    ...
    q_box_x_0 = 0.0
    ball_1_to_box_distance = 0.3
    ball_2_to_box_distance = 0.2
    ...
    # Solver settings
    tol = 1e-4
    maxiter = 5 # Low maxiter for MPC speed
    
    # U_init = jnp.zeros((n_u, N_horizon))
    key = jax.random.key(1)
    U_init = jax.random.uniform(key, shape=(n_u, N_horizon))*10
    ...
    # sigma = jnp.array([1.0, 1.0, 1.0])*1e-1
    k_spring = jnp.array([1.0, 1.0, 1.0])*500
    v_d = 0.1
    v_s = jnp.array([1.0, 1.0, 1.0])*1e2
    manipulator = MyPointMassBoxManipulatorSmooth(dt=dt, 
                                            ...
                                            k_spring=k_spring,
                                            v_d=v_d,
                                            v_s=v_s) # mu=0.0 for box-floor to slide
    ...
    for k in range(N_sim):
        ...
        # xkPlusOne = manipulator.f_fcn(current_x, uk)
