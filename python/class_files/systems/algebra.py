...
e1 = jnp.array([1, 0, 0], dtype=float)
e2 = jnp.array([0, 1, 0], dtype=float)
e3 = jnp.array([0, 0, 1], dtype=float)
...
def atan2(y, x):
    ...
    """
    if x > 0:
        return jnp.arctan(y / x)
    elif x < 0:
        if y >= 0:
            return jnp.arctan(y / x) + jnp.pi
        elif y < 0:
            return jnp.arctan(y / x) - jnp.pi
    else:
        # x == 0
