...
class System(ABC):
    ...
    def __init__(
        ...
        #  n_g: int, # Number of bilateral constraints
        dt: float,
        integrator: str = "rk4",
        ...
        if reg_friction is None:
            self.reg_friction = 1e-6 * jnp.ones(
                n_c,
            )
        else:
    ...
    # @abstractmethod
    # def _bilateral_constraint_function(self, q:jnp.ndarray, v: jnp.ndarray) -> jnp.ndarray:
    #     pass
    ...
    def _solve_contact_dynamics_jvp(self, primals, tangents):
        ...
        _, rhs_val = jax.jvp(
            lambda *p: total_smooth_residual(z_star_total, *p), params, d_params
        )
    ...
    def _solve_elastic_contact_dynamics(self, qk, vk, uk, r, dP_guess):
        ...
        def outer_loop_body(state):
            ...
            for i in range(self.n_c):
                ...
                xi_N = (
                    gamma_N_next
                    + e_restitution * self._contact_jacobian(qk_mid)[:, idx_n].T @ vk
                )
    ...
    def _solve_elastic_contact_dynamics_jvp(self, primals, tangents):
        ...
        # 1. Run Primal Solver
        qk1, vk1, dP_star = self._solve_elastic_contact_dynamics(
            self, qk, vk, uk, r, dP_guess
        )
    ...
    def _solve_moreau_contact_dynamics_jvp(self, primals, tangents):
        ...
        # 1. Run Primal Solver
        qk1, vk1, dP_star = self._solve_moreau_contact_dynamics(
            self, qk, vk, uk, r, dP_guess
        )
