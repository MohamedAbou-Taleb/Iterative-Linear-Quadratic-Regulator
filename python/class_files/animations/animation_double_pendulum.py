...
class AnimationDoublePendulum:
    ...
    def __init__(self, double_pendulum_sys, X_data, tspan, dt):
        ...
        (
            ...
            self.A_IB_1,
            self.A_IB_2,
        ) = self.compute_all_positions_and_orientations(self.q)
    ...
    def compute_positions(self, q):
        ...
        return np.array([x1, y1, np.zeros(len(x1))]), np.array(
            [x2, y2, np.zeros(len(x2))]
        )
    ...
    def compute_all_positions_and_orientations(self, q):
        ...
        A_IB_2 = np.array(
            [self.compute_transformation_matrix(q_i[0] + q_i[1]) for q_i in q.T]
        )
    ...
    def create_environment(self):
        ...
        wall.SetXLength(0.02)
        wall.SetYLength(2 * (self.l1 + self.l2))
        wall.SetZLength(0.2)
        _H_IB_wall = vtk.vtkMatrixToLinearTransform()
        _H_IB_wall.SetInput(self.H_IB_wall)
        tf_filter_wall = vtk.vtkTransformPolyDataFilter()
        tf_filter_wall.SetInputConnection(wall.GetOutputPort())
        tf_filter_wall.SetTransform(_H_IB_wall)
        mapper_wall = vtk.vtkPolyDataMapper()
        mapper_wall.SetInputConnection(tf_filter_wall.GetOutputPort())
        actor_wall = vtk.vtkActor()
        actor_wall.SetMapper(mapper_wall)
        actor_wall.GetProperty().SetColor([0, 0, 0])
        ...
        self.renderer.AddActor(actor_wall)
    ...
    def set_scene_to_timestep(self, i):
        ...
        for k in range(3):
            self.H_IB_1.SetElement(k, 3, r1[k])
            self.H_IB_joint_1.SetElement(k, 3, rj1[k])
            self.H_IB_2.SetElement(k, 3, r2[k])
            self.H_IB_joint_2.SetElement(k, 3, rj2[k])
            for j in range(3):
                self.H_IB_1.SetElement(k, j, A1[k, j])
                self.H_IB_joint_1.SetElement(k, j, A1[k, j])
                self.H_IB_2.SetElement(k, j, A2[k, j])
                self.H_IB_joint_2.SetElement(k, j, A2[k, j])
...
if __name__ == "__main__":
    ...
    anim.animate(save_video=False, filename="final_pendulum.mp4", fullscreen=True)
