...
class AnimationPointMassBox7DoF:
    ...
    def __init__(self, sys, X_data, tspan, dt):
        ...
        self.X_data = X_data.T
        # q indices: 
        # 0,1 (Ball1)
        # 2,3 (Ball2)
        # 4,5 (Box Position)
        # 6   (Box Orientation/Theta)
        ...
        self.r_ball1, self.r_ball2, self.r_box, self.theta_box_traj = self.compute_all_positions(self.q)
    ...
    def compute_all_positions(self, q):
        """
        Extracts position and orientation data from state vector q.
        q structure: [x_b1, y_b1, x_b2, y_b2, x_box, y_box, theta_box]
        """
        # Ball 1 (Indices 0, 1)
        x_b1 = q[0, :]
        y_b1 = q[1, :]
        ...
        # Box Position (Indices 4, 5)
        x_box = q[4, :]
        y_box = q[5, :]
        r_box = np.vstack([x_box, y_box, np.zeros(self.N)])

        # Box Orientation (Index 6)
        theta_box = q[6, :]

        return r_ball1, r_ball2, r_box, theta_box
    ...
    def create_environment(self):
        ...
        self.render_window.SetWindowName("7DoF Box and Point Mass Manipulation")
    ...
    def set_scene_to_timestep(self, i):
        ...
        # Retrieve orientation (theta)
        theta = self.theta_box_traj[i]
        c = np.cos(theta)
        s = np.sin(theta)

        # Update Ball 1 Matrix (Translation only)
        self.H_ball1.SetElement(0, 3, p_b1[0])
        self.H_ball1.SetElement(1, 3, p_b1[1])
        self.H_ball1.SetElement(2, 3, p_b1[2])

        # Update Ball 2 Matrix (Translation only)
        self.H_ball2.SetElement(0, 3, p_b2[0])
        self.H_ball2.SetElement(1, 3, p_b2[1])
        self.H_ball2.SetElement(2, 3, p_b2[2])

        # Update Box Matrix (Rotation + Translation)
        # 1. Rotation (Top-left 3x3)
        self.H_box.SetElement(0, 0, c)
        self.H_box.SetElement(0, 1, -s)
        self.H_box.SetElement(1, 0, s)
        self.H_box.SetElement(1, 1, c)
        
        # 2. Translation (Last column)
    ...
    def animate(
        self,
        save_video=False,
        filename="box_manipulation_7dof.mp4",
...
if __name__ == "__main__":
    # --- TEST STUB ---
    class MyBoxSystem7DoF:
        def __init__(self):
            self.ball_radius = 0.05
            self.box_width = 0.5
            self.box_height = 0.3

    sys_dummy = MyBoxSystem7DoF()
    ...
    # Box slides back and forth AND rotates
    x_box = 0.5 * np.sin(tspan)
    y_box = np.ones(N) * (sys_dummy.box_height / 2 + 0.2)
    
    # Rotation (The new DoF) - rotates back and forth
    theta_box = 0.5 * np.sin(2 * tspan)
    ...
    # Construct state [x_b1, y_b1, x_b2, y_b2, x_box, y_box, theta_box]
    q = np.vstack([x_b1, y_b1, x_b2, y_b2, x_box, y_box, theta_box])
    
    # Pad velocity (zeros) to make it look like full state X
    v = np.zeros_like(q)
    X_data = np.vstack([q, v]).T

    anim = AnimationPointMassBox7DoF(sys_dummy, X_data, tspan, dt)
